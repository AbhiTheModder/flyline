...
mod tests {
    ...
    fn create_test_git_repo() -> PathBuf {
        ...
        // git init -b master or fallback to git init
    ...
    fn test_find_git_repo_root() {
        ...
        let found = find_git_repo_root(&repo_dir);
    ...
    fn test_scan_and_apply_git_repo_payload() {
        reset_cache();
        let repo_dir = create_test_git_repo();
        let payload = scan_git_repo_payload(&repo_dir);
        ...
        let mtime = get_ref_mtime_in_dir(&repo_dir, "master");
        let mtime_main = get_ref_mtime_in_dir(&repo_dir, "main");
        let active_branch = if mtime.is_some() { "master" } else { "main" };
        let branch_mtime = mtime.or(mtime_main);
        assert!(branch_mtime.is_some());

        // Second scan with cached state present should return Unchanged
        let unchanged_payload = scan_git_repo_payload(&repo_dir);
        assert!(matches!(
            unchanged_payload,
        ...
        let mtime_space = get_ref_mtime_in_dir(&repo_dir, &format!("{} ", active_branch));
        assert_eq!(branch_mtime, mtime_space);

        // Test HEAD lookup
        let head_mtime = get_ref_mtime_in_dir(&repo_dir, "HEAD");
        assert!(head_mtime.is_some());
        let head_space_mtime = get_ref_mtime_in_dir(&repo_dir, "HEAD ");
        assert_eq!(head_mtime, head_space_mtime);

        // Test tag lookup
        let tag_mtime = get_ref_mtime_in_dir(&repo_dir, "v1.0.0");
        assert!(tag_mtime.is_some());

        // Test non-existent branch
        assert_eq!(
            get_ref_mtime_in_dir(&repo_dir, "non_existent_branch_12345"),
            None
        );
