...
RUN sh -c 'cc=$(find /android-ndk -name "x86_64-linux-android*-clang" | head -n1); "$cc" -shared -o /android-ndk/sysroot/usr/lib/x86_64-linux-android/libreadline.so -x c /dev/null' || true
RUN find /android-ndk -name "libc.so" -o -name "libc.a" -exec sh -c 'for f; do cp -f /android-ndk/sysroot/usr/lib/x86_64-linux-android/libreadline.so "$(dirname "$f")/" 2>/dev/null || true; done' _ {} +
